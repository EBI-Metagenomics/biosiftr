...
def accessory_writer( reps_clusters, loc_prefix ):
    for rep in reps_clusters:
        if len(reps_clusters[rep]) > 1:
            rep_prefix = rep[:-2]
            pan_loc = loc_prefix + '/species_catalogue/' + rep_prefix + '/' + rep + '/pan-genome/'
...
def annot_writer( reps_clusters, loc_prefix, pfam_desc ):
    for rep in reps_clusters:
        ...
        rep_prefix = rep[:-2]
        rep_loc = loc_prefix + '/species_catalogue/' + rep_prefix + '/' + rep + '/genome/'
        ...
        # Parsing the accesory genes annotation
            ...
            for member in relevant_members:
                all_loc = loc_prefix + '/all_genomes/' + rep_prefix + '/' + rep + '/genomes1/'
...
def main():
    ...
    if args.mode == 'pre':
        ...
        exit('Option '+args.mode+' is not valid. Provide a valid option on the mode argument: `pre` or `post`')
