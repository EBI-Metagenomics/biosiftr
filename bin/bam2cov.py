...
def bam_parser( bam_file ):
                ...
                if 'XA:Z:' in read.tostring():
                    ...
                    for alignment in extra_matches.split(';'):
                        if len(alignment) > 0:
                            ...
                            matches = sum(int(length) for length, op in re.findall(r'(\d+)([M=X])', cigar))
    ...
    return( all_matches, unique_matches, best_matches, multi_matches, ave_read_len, multimapping_counter )


def coverage_calc( out_root, genomes_len, all_matches, unique_matches, best_matches, multi_matches, ave_read_len, multimapping_counter ):
    ...
    with open(out_root+'_multimap_report.tsv','w') as report_out:
        ...
        report_out.write('Total number of targets with multimapping reads: '+str(targets_num)+'\n')
        report_out.write('Total number of multimapping reads: '+str(multimapping_counter)+'\n')

        sorted_dict = dict(sorted(multi_matches.items(), key=lambda item: item[1], reverse=True))
...
def main():
    ...
    ( all_matches, unique_matches, best_matches, multi_matches, ave_read_len, multimapping_counter ) = bam_parser( args.bwa_bam ) 

    coverage_calc( out_root, genomes_len, all_matches, unique_matches, best_matches, multi_matches, ave_read_len, multimapping_counter )
