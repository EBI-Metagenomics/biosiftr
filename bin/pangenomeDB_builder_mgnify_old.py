...
def accessory_writer(reps_clusters, panaroo_path):
    for rep in reps_clusters:
        if len(reps_clusters[rep]) > 1:
            ...
            genes_loc = pan_loc + "pan_genome_reference.fa"
...
def main():
    parser = argparse.ArgumentParser(
        description="This script find the accessory genes that needs eggNOG annotation for the human-gut catalogue"
    )
    parser.add_argument(
        "--metadata",
        type=str,
    ...
    if args.mode == "pre":
        accessory_writer(reps_clusters, args.panaroo_path)
    elif args.mode == "post":
        annot_writer(reps_clusters, loc_prefix, args.panaroo_path, pfam_desc)
    else:
        exit(
            "Option "
            + args.mode
